...
const LABELS: Record<string, string> = {
  ...
  NEGATIVE_PROJECTED_BALANCE: "Negative Projected Balance",
...
export default async function AlertSettingsPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-lg font-semibold text-navy mb-1">Alert Thresholds</h2>
        ...
        <div className="bg-white border border-border rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-surface text-left text-xs uppercase text-gray-500">
              <tr>
                <th className="px-4 py-2">Jenis Alert</th>
                <th className="px-4 py-2">Threshold Amount (Rp)</th>
                <th className="px-4 py-2">Threshold Jam</th>
      ...
      <div>
        <h2 className="text-sm font-semibold text-navy mb-3">Alert Terbuka</h2>
